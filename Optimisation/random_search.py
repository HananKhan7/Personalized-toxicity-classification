...
# Define the core and optional user profile traits
core_traits = ['education', 'age_range', 'lgbtq_status', 'race', 'toxic_comments_problem']
optional_traits = [
...
# Function to select random (unique) traits for each experiment
def select_random_traits(selected_traits_set):
    while True:
        num_optional_traits = random.randint(0, len(optional_traits))
        selected_optional_traits = random.sample(optional_traits, num_optional_traits)
        selected_traits = tuple(core_traits + selected_optional_traits)
...
def create_annotator_output(dataset, max_annotators, N_shots, output_path, batch_size, num_experiments):
    ...
    Parameters:
    dataset (list): A list of dictionaries, where each dictionary represents an annotator and their datasets. Each annotator dictionary should have the following keys:
    'annotator_id', 'annotator_profile', 'test_dataset', 'train_dataset', and 'secondary_annotators'.
    ...
    num_experiments (int): The number of random experiments to conduct.
    ...
    # Set to keep track of selected traits combinations
    selected_traits_set = set()

    for experiment in range(num_experiments):
        selected_traits = select_random_traits(selected_traits_set)
